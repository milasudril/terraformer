...
namespace terraformer
{
		...
		geosimd::rotation_angle src_dir)
	{
		...
		if(std::abs(dr[0]) > std::abs(dr[1]))
		{
				...
				l += dx)
			{
				...
				if(src_altitude < interp(heighmap, x, y) && l != x_0)
				{
					return pixel_coordinates{static_cast<uint32_t>(l),
		...
		else
		{
				...
				k += dy)
			{
				...
				if(src_altitude < interp(heighmap, x, y) && k != y_0)
				{
					return pixel_coordinates{static_cast<uint32_t>(x),
	...
	float source_visible(span_2d<float const> heighmap, int32_t x_0, int32_t y_0, direction src_dir)
	{
		...
		// FIXME This will not work for a close to vertical direction
