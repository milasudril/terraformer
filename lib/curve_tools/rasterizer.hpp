...
namespace terraformer
{
	...
	concept brush2 = requires(T obj, PixelType old_val, PixelType new_val, float x, float y, float z, float xi, float eta)
	{
	...
	template<class PixelType, brush2<PixelType> Brush>
	struct paint_params_2
	{
		float x;
		float y;
		PixelType value;
	...
	template<class PixelType, brush2<PixelType> Brush>
	void paint(span_2d<PixelType> target_surface, paint_params_2<PixelType, Brush> const& params)
	{
	...
	template<class PixelType, brush2<PixelType> Brush>
	struct line_segment_draw_params_2
	{
		PixelType value;
		float scale;
		Brush brush{};
	...
	void draw(span_2d<PixelType> target_surface,
		geosimd::line_segment<geom_space> seg,
		line_segment_draw_params_2<PixelType, Brush>& params)
	{
		auto dr = seg.p2 - seg.p1;
		if(std::abs(dr[0]) > std::abs(dr[1]))
		{
				...
				l += dx)
			{
				...
				paint(target_surface,
					paint_params_2<PixelType, Brush>{
						.x = x/params.scale,
						.y = y/params.scale,
