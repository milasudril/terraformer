...
)
{
...

	{
		...
		for(size_t k = 0; k != start_branches.branch_count; ++k)
		{
			...
			{ ret.left.push_back(points[indices.front()], 0.0f, v, indices.front(), 0.0f); }
			else
			{ ret.right.push_back(points[indices.front()], 0.0f, v, indices.front(), 0.0f); }
		}
	}
	...
	for(auto k : index_range{indices.front() + 1, indices.back() - 1})
	{
		if(l != std::size(x_intercepts.zeros) && k == displaced_curve::index_type{x_intercepts.zeros[l]})
		{
			if(selected_branch_point.has_value())
			{
				...
				{
					if(side >= 0.0f)
					{ ret.left.push_back(loc_b, 0.0f, *normal, *selected_branch_point, 0.0f); }
					else
					{ ret.right.push_back(loc_b, 0.0f, *normal, *selected_branch_point, 0.0f); }
				}
			}
			max_offset = 0.0f;
			++l;
...

	{
		...
		for(size_t k = 0; k != end_brancehs.branch_count; ++k)
		{
			...
			direction const v{last_tangent.apply(geosimd::rotation<geom_space>{theta, geosimd::dimension_tag<2>{}})};
			if(theta - ref_angle > geosimd::turn_angle{geosimd::turns{0.5}})
			{ ret.right.push_back(points[indices.back()], 0.0f, v, indices.back(), 0.0f); }
			else
			{ ret.left.push_back(points[indices.back()], 0.0f, v, indices.back(), 0.0f); }
		}
	}
...
terraformer::collect_branch_indices(ridge_tree_branch_seed_sequence_pair const& seq_pair)
{
	...
	std::ranges::copy(seq_pair.left.get<3>(), std::back_inserter(ret));
	std::ranges::copy(seq_pair.right.get<3>(), std::back_inserter(ret));
	std::ranges::sort(ret);
	return ret;
}
