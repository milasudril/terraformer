...
{
	auto image_a = load(std::type_identity<terraformer::image>{}, "test_pattern/test_pattern.exr");
	auto id_string = std::string{MAIKE_BUILDINFO_TARGETDIR "/"};
	id_string += std::to_string(MAIKE_TASKID);
	store(image_a, id_string.c_str());
	auto image_b = load(std::type_identity<terraformer::image>{}, id_string.c_str());
